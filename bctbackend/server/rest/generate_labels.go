...
func GenerateLabels(context *gin.Context, configuration *configuration.Configuration, db *sql.DB, userId models.Id, roleId models.RoleId) {
	...
	if err := context.ShouldBindJSON(&payload); err != nil {
		slog.Warn("Failed to parse payload for GenerateLabels endpoint", "error", err)
		failure_response.InvalidRequest(context, "Failed to parse payload:"+err.Error())
		return
	}

	if len(payload.ItemIds) == 0 {
		slog.Warn("Blocked attempt at generating labels for zero items", "userId", userId)
	...
	if err != nil {
		if errors.Is(err, dberr.ErrNoSuchItem) {
			slog.Warn("Blocked attempt at generating labels for non-existing items", "itemIds", payload.ItemIds, "userId", userId)
	...
	for _, item := range itemTable {
		if item.SellerID != userId {
			slog.Warn("Blocked attempt at generating labels for items not owned by the seller", "loggedInUserId", userId, "itemUserId", item.SellerID, "itemId", item.ItemID)
	...
	if err != nil {
		slog.Error("Failed to collect label data", "error", err)
