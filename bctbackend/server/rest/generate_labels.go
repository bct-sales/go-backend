...
func (endpoint *generateLabelsEndpoint) execute() {
	...
	if err := context.ShouldBindJSON(&payload); err != nil {
		logger.InvalidInput("Failed to parse payload for GenerateLabels endpoint", "error", err)
		failure_response.InvalidRequest(context, "Failed to parse payload:"+err.Error())
		return
	}

	if len(payload.ItemIds) == 0 {
		logger.InvalidRequest("Blocked attempt at generating labels for zero items", "userId", userId)
	...
	if err != nil {
		if errors.Is(err, dberr.ErrNoSuchItem) {
			logger.InvalidRequest("Blocked attempt at generating labels for non-existing items", "itemIds", payload.ItemIds, "userId", userId)
	...
	for _, item := range itemTable {
		if item.SellerID != userId {
			logger.InvalidRequest("Blocked attempt at generating labels for items not owned by the seller", "loggedInUserId", userId, "itemUserId", item.SellerID, "itemId", item.ItemID)
	...
	if err != nil {
		logger.InvalidRequest("Invalid layout for label generation", "error", err)
		failure_response.InvalidLayout(context, "Invalid label layout: "+err.Error())
		return
	}
