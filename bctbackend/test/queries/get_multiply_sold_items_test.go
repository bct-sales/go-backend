...
func TestGetMultiplySoldItems(t *testing.T) {
	...
	t.Run("Item sold thrice", func(t *testing.T) {
		...
		require.Equal(t, sale1.SaleID, multiplySoldItem.Sales[0].SaleID)
		require.Equal(t, sale2.SaleID, multiplySoldItem.Sales[1].SaleID)
		require.Equal(t, sale3.SaleID, multiplySoldItem.Sales[2].SaleID)
	})
	...
	t.Run("Two items sold twice", func(t *testing.T) {
		...
		require.Equal(t, sale1.SaleID, multiplySoldItems[0].Sales[0].SaleID)
		require.Equal(t, sale2.SaleID, multiplySoldItems[0].Sales[1].SaleID)

		require.Equal(t, *(items[1]), multiplySoldItems[1].Item)
		require.Len(t, multiplySoldItems[1].Sales, 2)
		require.Equal(t, sale1.SaleID, multiplySoldItems[1].Sales[0].SaleID)
		require.Equal(t, sale2.SaleID, multiplySoldItems[1].Sales[1].SaleID)
	})
	...
	t.Run("Sales [1], [1, 2], [1, 2, 3]", func(t *testing.T) {
		...
		require.Equal(t, sale1.SaleID, multiplySoldItems[0].Sales[0].SaleID)
		require.Equal(t, sale2.SaleID, multiplySoldItems[0].Sales[1].SaleID)
		require.Equal(t, sale3.SaleID, multiplySoldItems[0].Sales[2].SaleID)

		require.Equal(t, *(items[1]), multiplySoldItems[1].Item)
		require.Len(t, multiplySoldItems[1].Sales, 2)
		require.Equal(t, sale2.SaleID, multiplySoldItems[1].Sales[0].SaleID)
		require.Equal(t, sale3.SaleID, multiplySoldItems[1].Sales[1].SaleID)
	})
}
