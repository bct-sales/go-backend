...
func DefineEndpoints(db *sql.DB, router *gin.Engine, configuration *Configuration) {
	...
	router.GET(paths.Items().WithRawItemId(":id").String(), withUserAndRole(GetItemInformation))
	router.PUT(paths.Items().WithRawItemId(":id").String(), withUserAndRole(UpdateItem))
	...
	router.GET(paths.Sales().WithRawSaleId(":id").String(), withUserAndRole(GetSaleInformation))
